// js/men-shoes.js
// JK Fashion Empire - Men's Shoes page interactions
...
// - Quick preview (lightweight) for product images
...
(function () {
  ...
  /* ---------------------------
     Quick preview (lightweight)
     - Elements can use data-preview attribute to trigger
  ----------------------------*/
  (function setupQuickPreview() {
    function createPreview() {
      var overlay = document.createElement('div');
      overlay.id = 'shoe-preview';
      overlay.style-1.cssText = 'position:fixed;inset:0;display:none;align-items:center;justify-content:center;background:rgba(0,0,0,0.6);z-index:1200;padding:20px;';
      overlay.innerHTML = '<div style="max-width:900px;width:100%;background:#fff;border-radius:12px;overflow:hidden;position:relative;">' +
        '<button id="shoe-preview-close" aria-label="Close" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>' +
        '<img id="shoe-preview-img" src="" alt="Preview" style="width:100%;height:420px;object-fit:cover;display:block;" />' +
        '<div style="padding:14px;"><h3 id="shoe-preview-title" style="margin:0 0 6px 0;font-weight:800;"></h3><p id="shoe-preview-price" style="margin:0;color:#666;font-weight:700;"></p></div>' +
        '</div>';
      document.body.appendChild(overlay);

      overlay.querySelector('#shoe-preview-close').addEventListener('click', function () {
    ...
    var overlay = document.getElementById('shoe-preview') || createPreview();
    ...
    document.addEventListener('click', function (e) {
      ...
      var previewImg = overlay.querySelector('#shoe-preview-img');
      var previewTitle = overlay.querySelector('#shoe-preview-title');
      var previewPrice = overlay.querySelector('#shoe-preview-price');
